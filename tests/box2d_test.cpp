...
TEST(Box2D_test, test_construction)
{
  ...
  Box2D box7(1.0, 2.0, 3.0);
  EXPECT_EQ(box7.LeftBottom(), Point2D(1.0, 0.0));
  EXPECT_EQ(box7.RightTop(), Point2D(3.0, 2.0));
}

TEST(Box2D_test, test_tips)
{
  Box2D box = { 0.0, 0.0, 1.0, 1.0 };
...
TEST(Box2D_test, test_initializer_list)
{
  ...
  EXPECT_EQ(box2.LeftBottom(), Point2D(0.0, 0.0));
  EXPECT_EQ(box2.RightTop(), Point2D(1.0, 8.0));

  Box2D box3 = {0.0, 5.0, 7.0, 8.0, 1.0};
  EXPECT_EQ(box3.LeftBottom(), Point2D(0.0, 5.0));
  EXPECT_EQ(box3.RightTop(), Point2D(7.0, 8.0));


  Box2D box4 = {2.0, 3.0, 4.0};
  EXPECT_EQ(box4.LeftBottom(), Point2D(2.0, 0.0));
  EXPECT_EQ(box4.RightTop(), Point2D(4.0, 3.0));
}
...
{
  Box2D box1 = { Point2D(0.0, 0.0), Point2D(5.0, 5.0) };
  Box2D box2 = { 3.0, 3.0, 8.0, 8.0 };
